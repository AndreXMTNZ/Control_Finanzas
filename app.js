// app.js (compatibilidad total móvil - sin <dialog>)
...
btnNewProfile?.addEventListener("click", async () => {
  setMsg(profileMsg, "");
  const name = prompt("Nombre del perfil (ej: Andrés):");
  if (!name) return;
  ...
  try {
    ...
    console.error(err);
    setMsg(profileMsg, "Error: " + (err?.message || err));
  }
});

/** ✅ PERFIL: editar (PROMPT) */
btnEditProfile?.addEventListener("click", async () => {
  setMsg(profileMsg, "");
  const id = profileSelect.value;
  if (!id) return;

  const currentName = profileSelect.options[profileSelect.selectedIndex]?.textContent || "";
  const newName = prompt("Editar nombre del perfil:", currentName);
  if (!newName) return;

  try {
    await updateDoc(doc(db, "profiles", id), { name: newName.trim() });
...
/** Init */
loadProfiles();const profileMsg = document.getElementById("profileMsg");
